...
class Theplus_Widgets_Rules extends Elementor\Widget_Base {
	...
	public function add_rules_controls( $element, $args ) {
		...
		$repeater->add_control(
            'tp_rule_location_apitype',
            [
                'type'          => Controls_Manager::SELECT,
                'label_block'   => true,
                'multiple'      => false,
                'options' => [
                    'geoplugin' => esc_html__( 'Geoplugin', 'theplus' ),
                    'ip-api' => esc_html__( 'IP API', 'theplus' ),
                    'findip' => esc_html__( 'Find IP', 'theplus' ),
                ],
                'default'       => 'ip-api',
                'condition'     => [
                    'tp_rule_key' => 'location',
	...
	protected function set_rules( $id, $rules = [] ) {
		...
		foreach ( $rules as $index => $rule ) {
			...
			if ( method_exists( $this, 'plus_check_' . $key ) ) {
                if( 'plus_check_' . $key === 'plus_check_location' ){
                    $check = $this->plus_check_location( $value, $check_is_not, $key_name, $rule );
     ...
     * Check location
     *
     * For Testing use $ip this 207.0.0.1
     *
     * @access protected
     *
     * @param mixed  $value  The control value to check
     ...
     * https://www.geoplugin.com/iso3166
     */
	protected static function plus_check_location( $value, $check_is_not, $key, $rule ){   
        $display = false;       
        $ip = '';   // For Testing use this value 207.0.0.1
        ...
        $location_apitype = !empty($rule['tp_rule_location_apitype']) ? $rule['tp_rule_location_apitype'] : '';
        ...
        $url = '';
        if ( 'geoplugin' === $location_apitype ){
            $url = "http://www.geoplugin.net/php.gp?ip=$ip";
        } else if ( 'ip-api' === $location_apitype ) {
            $url = "http://ip-api.com/php/$ip";
        } else if ( 'findip' === $location_apitype ) {
            $url = "https://api.findip.net/$ip/?token=50f12f3fdd02461ca77effb7ce08858e";
        }
        ...
        if ( $response_code === 200 ) {
            $data1 = wp_remote_retrieve_body($response);
        } else {
        ...
        if( !empty( $data1 ) ){
            if ( 'geoplugin' === $location_apitype ){
                ...
                $countryName = !empty($data['geoplugin_countryName']) ? strtolower( $data['geoplugin_countryName'] ) : ''; 
            } else if ( 'findip' === $location_apitype ) {
                $data = json_decode( $data1, true );
                self::$tmp_location = $data;

                $countryName = !empty($data['country']['names']['en']) ? strtolower( $data['country']['names']['en'] ) : '';
            }else{
                $data = unserialize($data1);
                self::$tmp_location = $data;
                ...
                $countryName = !empty($data['country']) ? strtolower( $data['country'] ) : '';
            }

			echo "<script> console.log( '" . $countryName . "' ); </script>";
