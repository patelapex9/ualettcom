...
class ThePlus_MorphingLayouts extends Widget_Base {
	...
	public function get_keywords() {
		return array( 'morphing', 'morphing sections', 'blob section', 'blob builder', 'SVG Sections' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			array(
				...
				'separator' => 'before',
				'condition' => array(
					'morph_layout' => array( 'sec_bg', 'column_bg', 'fixed_scroll' ),
				),
			)
		);
		...
		$repeater->add_control(
			...
			array(
				...
				'options' => $this->theplus_anime_animation_easing(),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'separator' => 'before',
				'condition' => array(
					'morph_layout' => array( 'fixed_scroll' ),
				),
			)
		);
		$this->add_group_control(
			...
			array(
				...
				'condition' => array(
					'morph_layout'          => array( 'fixed_scroll' ),
					'morph_fixed_scroll_bg' => array( 'yes' ),
				),
			)
		);
	...
	protected function render() {
		...
		$morph_layout = ! empty( $settings['morph_layout'] ) ? $settings['morph_layout'] : 'normal';
		$morph_type   = ! empty( $settings['morph_type'] ) ? $settings['morph_type'] : 'color';

		if ( 'sec_bg' === $morph_layout ) {
			$data_class .= ' morph-row';
			$data_attr  .= ' data-morphrow="yes"';
		}

		if ( 'column_bg' === $morph_layout ) {
		...
		if ( ! empty( $settings['morphing_loop'] ) ) {
			foreach ( $settings['morphing_loop'] as $index => $item ) {
				$fixed_scroll_alt = ! empty( $item['fixed_scroll_alt'] ) ? $item['fixed_scroll_alt'] : 'none';

				if ( 'custom' !== $item['layouts'] ) {
					if ( $i > 0 || ( 'fixed_scroll' === $morph_layout ) ) {
						$json_arr['path'][] = $this->get_morphing_style( $item['layouts'] );
					} elseif ( 0 == $i ) {
						$first_morph = $this->get_morphing_style( $item['layouts'] );
					}
					$morph_style = 'yes';
				} elseif ( $i > 0 || ( 'fixed_scroll' === $morph_layout ) ) {
					$json_arr['path'][] = wp_kses_post( $item['custom_layouts'] );
				} elseif ( 0 == $i && ( 'fixed_scroll' !== $morph_layout ) ) {
					$first_morph = wp_kses_post( $item['custom_layouts'] );
				}

				if ( 'custom' !== $fixed_scroll_alt && 'none' !== $fixed_scroll_alt ) {
					if ( $i > 0 || ( 'fixed_scroll' === $morph_layout ) ) {
						$json_arr['pathAlt'][] = $this->get_morphing_style( $fixed_scroll_alt );
					} elseif ( 0 == $i ) {
						$first_morph_alt = $this->get_morphing_style( $fixed_scroll_alt );
					}
				} elseif ( 'custom' === $fixed_scroll_alt ) {
					if ( $i > 0 || ( 'fixed_scroll' === $morph_layout ) ) {
				...
				if ( ! empty( $item['fill_color_loop'] ) && 'yes' === $item['fill_color_loop'] ) {
		...
		$shape_trasnform = '';
		if ( 'yes' === $morph_style || 'yes' === $settings['custom_morph_path_blobmaker'] ) {
		...
		if ( 'gradient' === $morph_type ) {
			...
			if ( 'yes' === $settings['image_on_hover'] ) {
			...
			$image_json_array = wp_json_encode( $image_json );
		...
		$output .= '<svg class="morph" width="' . esc_attr( $morph_width ) . '" height="' . esc_attr( $morph_height ) . '" viewBox="0 0 ' . esc_attr( $viewbox_width ) . ' ' . esc_attr( $viewbox_height ) . '">';
		$output .= $clippath_st . $gradient_svg . '<path ' . $fill_style . ' d="' . $first_morph . '" ' . $shape_trasnform . '  />' . $clippath_end . $image_clip;
		$output .= '</svg>';
		$output .= '</div>';

		if ( 'yes' === $settings['morph_fixed_scroll_bg'] && 'fixed_scroll' === $morph_layout ) {
	...
	/**
	 * Render Morphing style
	 *
	 * @since 2.0.0
	 *
	 * @param string $style The attribute slug for style.
	 *
	 * @version 5.4.2
	 */
	...
	/**
	 * Get theplus_anime_animation_easing
	 *
	 * @since 2.0.0
	 */
	public function theplus_anime_animation_easing() {
		return array(
