		...
		if ( ! empty( $settings['loop_content'] ) ) {
			...
			foreach ( $loop_content as $index => $item ) {
				...
				if ( ! empty( $item['loop_title'] ) ) {
					...
					$list_title  = $title_a_start . '<' . theplus_validate_html_tag( $heading_tag ) . ' class="tp-pro-step-title">' . wp_kses_post( $item['loop_title'] ) . '</' . theplus_validate_html_tag( $heading_tag ) . '>' . $title_a_end;
				}
				...
				if ( ! empty( $item['loop_image_icon'] ) ) {
					...
					if ( isset( $item['loop_image_icon'] ) && 'image' === $item['loop_image_icon'] ) {
						...
						if ( ! empty( $item['loop_icon_style'] ) && 'icon_mind' === $item['loop_icon_style'] ) {
