...
class ThePlus_Progress_Bar extends Widget_Base {
	...
	protected function register_controls() {
		...
		$this->add_group_control(
			...
			array(
				...
				'default'   => 'full',
				'separator' => 'none',
				'condition' => array( 'image_icon' => 'image' ),
			)
		);
		$this->add_control(
			'type',
		...
		$this->add_control(
			...
			array(
				...
				'condition' => array(
					'image_icon' => array( 'icon', 'image', 'svg', 'lottie' ),
				),
			)
		);
		$this->add_control(
	...
	protected function render() {
		...
		$icon_postition = ! empty( $settings['icon_postition'] ) ? $settings['icon_postition'] : 'before';
		...
		$progress_bar_img = '';
		if ( 'image' === $image_icon && ! empty( $settings['select_image']['url'] ) ) {
			$select_image = $settings['select_image']['id'];
			$add_image    = tp_get_image_rander( $select_image, $settings['select_image_thumbnail_size'], array( 'class' => 'progress_bar-img' ) );

			$progress_bar_img = '<span class="progres-ims">' . $add_image . '</span>';
		}

		$icons = '';
		if ( 'icon' === $image_icon ) {
			$type = ! empty( $settings['type'] ) ? $settings['type'] : 'font_awesome';

			if ( 'font_awesome' === $type ) {
				$icons = $settings['icon_fontawesome'];
			} elseif ( 'font_awesome_5' === $type ) {
				ob_start();
				\Elementor\Icons_Manager::render_icon( $settings['icon_fontawesome_5'], array( 'aria-hidden' => 'true' ) );
				$icons = ob_get_contents();
				ob_end_clean();
			} elseif ( 'icon_mind' === $type ) {
				$icons = $settings['icons_mind'];
			}

			if ( 'font_awesome_5' === $type && ! empty( $settings['icon_fontawesome_5'] ) ) {
		...
		if ( ! empty( $settings['symbol'] ) ) {
			$symbol_position = ! empty( $settings['symbol_position'] ) ? $settings['symbol_position'] : 'after';

			if ( 'after' === $symbol_position ) {
				$symbol2 = '<span class="theserivce-milestone-number icon-milestone" data-counterup-nums="' . esc_attr( $settings['number'] ) . '">' . wp_kses_post( $settings['number'] ) . '</span><span class="theserivce-milestone-symbol">' . wp_kses_post( $settings['symbol'] ) . '</span>';
			} elseif ( 'before' === $symbol_position ) {
				$symbol2 = '<span class="theserivce-milestone-symbol">' . wp_kses_post( $settings['symbol'] ) . '</span><span class="theserivce-milestone-number" data-counterup-nums="' . esc_attr( $settings['number'] ) . '">' . wp_kses_post( $settings['number'] ) . '</span>';
			}
		} else {
			$symbol2 = '<span class="theserivce-milestone-number icon-milestone" data-counterup-nums="' . esc_attr( $settings['number'] ) . '">' . wp_kses_post( $settings['number'] ) . '</span>';
		}

		if ( 'gradient' === $settings['pie_fill'] ) {
			...
			if ( 'style_1' === $progressbar_style ) {
				if ( 'large' !== $progress_bar_size ) {
					...
					$progress_bar     .= '<div class="progress_bar-skill skill-fill ' . esc_attr( $progress_bar_size ) . '" style="background-color:' . esc_attr( $progress_empty_color ) . '">';
						$progress_bar .= '<div class="progress_bar-skill-bar-filled ' . esc_attr( $lz1 ) . '" data-width="' . esc_attr( $progress_width ) . '">	</div>';
					$progress_bar     .= '</div>';
				} else {
					$progress_bar             .= '<div class="progress_bar-skill skill-fill ' . esc_attr( $progress_bar_size ) . '" style="background-color:' . esc_attr( $progress_empty_color ) . '" >';
						$progress_bar         .= '<div class="progress_bar-skill-bar-filled ' . esc_attr( $lz1 ) . '" data-width="' . esc_attr( $progress_width ) . '">	</div>';
